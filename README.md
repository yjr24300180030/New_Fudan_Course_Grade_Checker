...
> **迁移方式**：
> 点击`Merge`按钮，将我的仓库的最新代码合并到你自己的仓库的main分支即可。
...
> **关于排名**：院系排名与专业排名均为零配置。排名接口只需学号对应的内部成绩单 ID 即可返回全院系同学的完整数据（其中每行都带 `major` 字段，仅你自己的信息为明文），专业排名由本地按专业筛选后重排得出，无需任何额外参数或专业内部 ID。
